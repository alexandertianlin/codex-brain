...
SKILL_SYNTHESIS_TERMS = {
    "nuwa", "女娲", "造skill", "造 skill", "造人", "蒸馏", "人物skill", "人物 skill",
    "perspective", "思维框架", "思维方式", "更新skill", "更新 skill", "skill synthesis",
}
LEARNING_TERMS = {
...
def classify(task: str, project: str = "") -> dict:
    ...
    if _has(text, SKILL_SYNTHESIS_TERMS):
...
def _risk(text: str) -> str:
    risk = "inspect"
    if any(word in text for word in ["修改", "fix", "改代码", "实现", "执行", "安装", "部署到本地"]):
        risk = "modify"
    if any(word in text for word in ["unity play", "硬件", "真实", "发送", "push", "deploy", "commit", "删除"]):
...
def _protected_baselines(text: str) -> list[str]:
    protected = []
    if "2.2" in text:
        protected.append(r"D:\agiletact\onlytip - 2.2-mediapipe")
    if "v4" in text:
        protected.append("HaMeR V4 stable variants")
    if "onlytip" in text and "稳定" in text:
